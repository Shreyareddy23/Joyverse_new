...
interface Session {
  ...
  tracingResults?: Array<{
    letter: string;
    imageData?: string;
    tracedAt: string | Date;
  }>;
}
...
const TherapistDashboard: React.FC = () => {
  ...
  const [childGameSelection, setChildGameSelection] = useState<Record<string, ('typing' | 'puzzles' | 'reading' | 'tracing')[]>>({});
  ...
  const [tracingResultsLoading, setTracingResultsLoading] = useState(false);
  const [tracingResultsData, setTracingResultsData] = useState<Array<{
    sessionId: string;
    date: string | Date;
    tracingResults: Array<{
      letter: string;
  ...
  const fetchTherapistData = async (username: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // initialize per-child game selection from assignedGames array
        const mapping: Record<string, ('typing' | 'puzzles' | 'reading' | 'tracing')[]> = {};
        (data.children || []).forEach((c: any) => {
          // Use assignedGames array if available, otherwise fall back to preferredGame
          if (c.assignedGames && Array.isArray(c.assignedGames) && c.assignedGames.length > 0) {
            mapping[c.username] = c.assignedGames.filter((g: string) => 
              ['typing', 'puzzles', 'reading', 'tracing'].includes(g)
            ) as ('typing' | 'puzzles' | 'reading' | 'tracing')[];
          } else if (c.preferredGame) {
            mapping[c.username] = [c.preferredGame] as ('typing' | 'puzzles' | 'reading' | 'tracing')[];
          } else {
            mapping[c.username] = [];
          }
        });
  ...
  const savePreferredStory = async (childUsername: string, storyId: string) => {
    try {
      ...
      // Update local game selection state to include reading
      setChildGameSelection(prev => {
        const currentGames = prev[childUsername] || [];
        if (!currentGames.includes('reading')) {
          return {
            ...prev,
            [childUsername]: [...currentGames, 'reading']
          };
        }
        return prev;
      });
  ...
  // Fetch tracing results for a child
  const fetchTracingResults = async (childUsername: string) => {
    setTracingResultsLoading(true);
    setTracingResultsData([]);
    try {
      // Get all sessions for this child
      const child = children.find(c => c.username === childUsername);
      if (!child || !child.sessions) {
        setTracingResultsData([]);
        setShowTracingModal(true);
        setTracingResultsLoading(false);
        return;
      }

      // Filter sessions that have tracing results
      const sessionsWithTracing = child.sessions
        .filter(session => session.preferredGame === 'tracing' && session.tracingResults && session.tracingResults.length > 0)
        .map(session => ({
          sessionId: session.sessionId,
          date: session.date,
          tracingResults: session.tracingResults || []
        }))
        .sort((a, b) => {
      ...
      setTracingResultsData(sessionsWithTracing);
      setShowTracingModal(true);
    } catch (err) {
      console.error('Failed to fetch tracing results', err);
      setTracingResultsData([]);
      setShowTracingModal(true);
    } finally {
      setTracingResultsLoading(false);
  ...
  return (
    <Container>
                ...
                >
                  <ChildCardHeader>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginRight: '140px' }}>
                        <SmallGameButton
                          active={(childGameSelection[child.username] || []).includes('typing')}
                          onClick={(e) => {
                            e.stopPropagation();
                            const currentGames = childGameSelection[child.username] || [];
                            const newGames = currentGames.includes('typing')
                              ? currentGames.filter(g => g !== 'typing')
                              : [...currentGames, 'typing'];
                            setChildGameSelection({ ...childGameSelection, [child.username]: newGames });
                          }}
                        >
                          Typing
                        </SmallGameButton>
                        <SmallGameButton
                          active={(childGameSelection[child.username] || []).includes('puzzles')}
                          onClick={(e) => {
                            e.stopPropagation();
                            const currentGames = childGameSelection[child.username] || [];
                            const newGames = currentGames.includes('puzzles')
                              ? currentGames.filter(g => g !== 'puzzles')
                              : [...currentGames, 'puzzles'];
                            setChildGameSelection({ ...childGameSelection, [child.username]: newGames });
                          }}
                        >
                          Puzzles
                        ...
                        <SmallGameButton
                          active={(childGameSelection[child.username] || []).includes('reading')}
                          onClick={async (e) => {
                            e.stopPropagation();
                            const currentGames = childGameSelection[child.username] || [];
                            const newGames = currentGames.includes('reading')
                              ? currentGames.filter(g => g !== 'reading')
                              : [...currentGames, 'reading'];
                            setChildGameSelection({ ...childGameSelection, [child.username]: newGames });
                            // If adding reading, open story selection modal
                            if (!currentGames.includes('reading')) {
                        ...
                        <SmallGameButton
                          active={(childGameSelection[child.username] || []).includes('tracing')}
                          onClick={(e) => {
                            e.stopPropagation();
                            const currentGames = childGameSelection[child.username] || [];
                            const newGames = currentGames.includes('tracing')
                              ? currentGames.filter(g => g !== 'tracing')
                              : [...currentGames, 'tracing'];
                            setChildGameSelection({ ...childGameSelection, [child.username]: newGames });
                          }}
                        >
                          Tracing
                    ...
                    <RightControls>
                      <SaveChoiceButton
                        disabled={(childGameSelection[child.username] || []).length === 0}
                        onClick={async (e) => {
                          e.stopPropagation();
                          const selectedGames = childGameSelection[child.username] || [];
                          try {
                            // Send all selected games in a single API call
                            ...
                            // Refresh data to get updated assignedGames
                            await fetchTherapistData(therapistUsername);
                            
                            // If puzzles is selected, navigate to theme assignment
                            if (selectedGames.includes('puzzles')) {
                              sessionStorage.setItem('selectedGame', 'puzzles');
                              sessionStorage.setItem('selectedGame_for', child.username);
                              sessionStorage.setItem('selectedChild', child.username);
                              sessionStorage.setItem('selectedChildTherapistCode', therapistCode);
                              navigate('/theme-assignment');
                            } else {
                              const gameNames = selectedGames.join(', ');
                              toast.showToast(`Saved ${gameNames} ${selectedGames.length > 1 ? 'activities' : 'activity'} for ${child.username}`);
                            }
                          } catch (err) {
                            console.error('Failed to save preferred games', err);
                            toast.showToast('Failed to save games. Please try again.');
                          }
                        }}
                      >
                        Save
                  ...
                  {/* Show action buttons based on assigned games */}
                  {(childGameSelection[child.username] || []).includes('puzzles') && (
                    <>
                      <ActionButton
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  {(childGameSelection[child.username] || []).includes('reading') && (
                  ...
                  {(childGameSelection[child.username] || []).includes('tracing') && (
                    <ActionButton
                      onClick={async (e) => {
                        e.stopPropagation();
                        // Show tracing results for tracing-mode child
                        setSelectedChild(child);
                        fetchTracingResults(child.username);
                      }}
                      style={{ marginTop: '10px' }}
                    >
                  ...
                  {(childGameSelection[child.username] || []).includes('typing') && (
                    <ActionButton
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  {(childGameSelection[child.username] || []).length === 0 && (
                    <div style={{ color: '#777', padding: '8px 0' }}>Select games for this child to enable actions</div>
                  )}
                </ChildCard>
  
                {selectedChild?.username === child.username && (childGameSelection[child.username] || []).includes('puzzles') && (
                  <SessionsContainer>
                              ...
                              {isExpanded && (
                                <SessionBody>
                                  ...
                                  {session.preferredGame === 'tracing' && (
                                    <>
                                      <SessionSection>
                                        <SectionTitle>Letter Tracing Results</SectionTitle>
                                        {session.tracingResults && session.tracingResults.length > 0 ? (
                                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '15px' }}>
                                            {session.tracingResults.map((trace, i) => (
                                              <div key={i} style={{ 
                                                padding: '12px', 
                                                background: '#f8f9fa', 
                                                borderRadius: '8px', 
                                                ...
                                                {trace.imageData && (
                                                  <img 
                                                    src={trace.imageData} 
                                                    alt={`Traced letter ${trace.letter}`}
                                                    style={{
                                                      width: '100%',
                                                      height: 'auto',
                                                      maxHeight: '200px',
                                                ...
                                                {trace.tracedAt && (
                                                  <div style={{ fontSize: '11px', color: '#999', marginTop: '8px' }}>
                                                    {new Date(trace.tracedAt).toLocaleDateString()} {new Date(trace.tracedAt).toLocaleTimeString()}
                                                  </div>
                                                )}
                                              </div>
                                            ))}
                                          ...
                                          <div style={{ color: '#666' }}>No tracing results in this session</div>
                                        )}
                                      </SessionSection>
                                    </>
                                  )}
                            ...
                            <div key={idx} style={{ 
                              ...
                              <div style={{ 
                                ...
                                <div style={{ 
                                  ...
                                  📖 Story: {recording.storyTitle}
                                </div>
                                {recording.storyContent && (
                                  <div style={{
                                    marginTop: '10px',
                                    marginBottom: '10px',
                                    padding: '12px',
                                    ...
                                    <div style={{ fontWeight: '600', marginBottom: '6px', color: '#555' }}>Story Content:</div>
                                    <div>{recording.storyContent}</div>
                                  </div>
                                )}
                                {recording.storyMoral && (
                                  <div style={{
                                    marginTop: '8px',
                                    padding: '10px',
                                    background: '#fff3e0',
                                    ...
                                    <strong>Moral:</strong> {recording.storyMoral}
                                  </div>
                                )}
      ...
      {/* Tracing Results Modal */}
      {showTracingModal && (
        <TypingModalOverlay>
          <TypingModal style={{ maxWidth: '900px' }}>
            <ModalCloseButton onClick={() => setShowTracingModal(false)}>×</ModalCloseButton>
            <ModalTitle>Tracing Results for {selectedChild?.username}</ModalTitle>
            {tracingResultsLoading ? (
              <div>Loading tracing results...</div>
            ) : tracingResultsData.length === 0 ? (
              <div style={{ color: '#666', padding: '20px', textAlign: 'center' }}>
                No tracing results available for this child yet.
              </div>
            ) : (
              <div style={{ maxHeight: '70vh', overflow: 'auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                {tracingResultsData.map((session) => (
                  <div key={session.sessionId} style={{
                    background: '#f8f9fa',
                    padding: '20px',
                    borderRadius: '12px',
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '20px' }}>
                      {session.tracingResults.map((trace, idx) => (
                        <div key={idx} style={{
                          background: 'white',
                          padding: '15px',
                          borderRadius: '8px',
                          ...
                          <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#667eea', marginBottom: '12px' }}>
                            Letter: {trace.letter}
                          </div>
                          {trace.imageData && (
                            <img 
                          ...
                          {trace.tracedAt && (
                            <div style={{ fontSize: '11px', color: '#999', marginTop: '8px' }}>
                              Traced on: {new Date(trace.tracedAt).toLocaleDateString()} {new Date(trace.tracedAt).toLocaleTimeString()}
                            </div>
                          )}
                        </div>
                      ))}
