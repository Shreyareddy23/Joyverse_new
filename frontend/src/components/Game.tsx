...
const adjustDifficulty = async () => {
  ...
  const handleHomeClick = async () => {
    ...
    // Mark puzzles game as completed if all puzzles are completed
    if (showThemeComplete && playedCount >= totalPuzzles) {
      try {
        const childData = sessionStorage.getItem('childData');
        if (childData) {
          const { username, therapistCode } = JSON.parse(childData);
        ...
        console.error('Failed to mark puzzles game as completed:', err);
      }
    }
